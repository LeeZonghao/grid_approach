...
def preprocessing(V, e, k, b, theta):
    ...
    if Points_in_box.size>0:
        ...
        print("#### difference are", np.allclose(D_test_2,D_2))
        print("#### finish,D_2")
        L_BB_test = np.zeros((m, m))
        L_B_test = np.zeros(r)
        L_B=L_B_cal(Points_in_box,k,L_B,G_2,D_2)
        L_B_test=L_B_cal(Points_in_box,k,L_B_test,G_2,D_test_2)
        print("#### difference are", np.allclose(L_B_test,L_B))
        print("#### finish,L_B")
    D_test_1=D_1_test(V,C,k,e,m,A,A_short,r)
    D_1=D_1_cal(V,C_short,k,e,position_matrix,m)
    print("#### difference are", np.allclose(D_test_1,D_1))
    print("#### finish,D_1")
    L_BB=L_BB_cal(m,k,L_BB,G_1,D_1)
    L_BB_test=L_BB_cal(m,k,L_BB_test,G_1,D_test_1)
    print("#### difference are", np.allclose(L_BB_test,L_BB))
    print("#### finish,L_BB")
    ...
    L_test=sum_log_likelihood(L_BB_test,L_B_test)
    print("#### difference are", np.abs(L_test-L))
...
@njit(parallel=True)
def up_likelihood1(proposed_v, e, theta, i, log_up, D,  fctn,V):
    r = D.shape[0]
    for j in prange(r):
...
r=V.shape[0]
D=np.zeros((r,r))
Log_like,D=actual_likelihood(V, e, theta,D,fctn=g_np_c)
L_true=np.sum(Log_like)
print("actual error are:" , np.abs(L_true-L))
...
for i in range(m-1):
  ...
  for j in range(i+1,m):
    ...
    for j_idx in select_box_j:
      for i_idx in select_box_i:
        print(np.linalg.norm(C_short[i,:]-C_short[j,:])-np.linalg.norm(C[i_idx,:]-C[j_idx,:]))
# k_near_matrix,k_dist_matrix,test_statistic,V=cal_posterior_prob(V,k,b,theta,e,20,L_BB, L_B, L, M_1,M_2,M_3,M_4, D_1, D_2, G_1, G_2, D_center, diff, A, C_short,N,Points_in_box,inv_factorial,binom_coeff,position_matrix,precomp_for_delta2,precomp_partitions_2,test_statistic,rank_matrix,fctn=g_np_c)
